...
function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="bg-black absolute inset-0 -z-10 w-full h-full" aria-hidden>
      <Canvas camera={camera} gl={{ antialias: true, alpha: false }} dpr={[1, 2]} style={{ width: "110%", height: "150%" }}>
        <ShaderPlane />
      </Canvas>
...
export default function NeuralNetwork({
  ...
  return (
    <section ref={sectionRef} className="relative   h-screen w-screen overflow-hidden">
      <div className="absolute inset-0 hue-rotate-140  saturate-0 brightness-300">
        <ShaderBackground />
      </div>
    </section>
  );
